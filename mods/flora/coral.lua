	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if def_top and def_top.liquidtype == "source" and minetest.get_item_group(node_top.name, "water") > 0 then
			if not minetest.is_protected(pos, player_name) and not minetest.is_protected(pos_top, player_name) then
				minetest.set_node(pos, { name = "flora:sand_with_kelp", param2 = height * 16 })
...
local function coral_on_place(itemstack, placer, pointed_thing)
	...
	if node_under.name ~= "flora:coral_skeleton" or minetest.get_node(pos_above).name ~= "blocks:water_source" then
		return itemstack
	end
