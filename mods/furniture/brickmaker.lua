...
local function brickmaker_show_formspec(pos, player, add)
	local playername = player:get_player_name()
	minetest.show_formspec(playername, "furniture:brickmaker", brickmaker_formspec_crafting(pos, player, add))
end
...
for i=1,#furniture.craftstation_materials do
	...
	minetest.register_node("furniture:brickmaker_" .. sname, {
		...
		on_construct = brickmaker_on_construct,
		after_place_node = brickmaker_after_place_node,
		on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
			...
			brickmaker_show_formspec(pos, clicker)
			furniture.brickmaker[playername] = pos
		end,
