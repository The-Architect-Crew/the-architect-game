...
function tavelnet.get_network_owner(netid)
    return string.match(travelnet.storage:get_string("network_" .. netid), ':(.*)'):split(",")[2]
end

function tavelnet.get_network_users(netid)
    return string.match(travelnet.storage:get_string("network_" .. netid), '(.*):'):split(",")
end

function tavelnet.get_station_name(statid)
    return string.match(travelnet.storage:get_string("station_" .. statid), ':(.*)'):split(",")[1]
end

function tavelnet.get_station_owner(statid)
    return string.match(travelnet.storage:get_string("station_" .. statid), ':(.*)'):split(",")[2]
end

function tavelnet.get_station_pos(statid)
    return minetest.string_to_pos(travelnet.storage:get_string("pos_" .. statid))
end

function tavelnet.set_network_name(netid, name)
...
function tavelnet.set_network_users(netid, users)
