...
local function add_home(player, hname, htype)
	...
	if homesdata[hname] then -- ensure name isnt already registered
		homes_chat(name, hname.." is already a registered "..htype.."! Please pick another name or delete the existing entry.")
		return
	end
	local ppos = player:get_pos()
	local order = homes_amt + 1
...
local function delete_home(player, hname)
	...
	if homesdata[hname] then
		...
		homesdata[hname] = nil
		meta:set_string("whomes", minetest.serialize(homesdata))
		homes_chat(name, "Removed '"..hname.."' from your list of homes!")
	else
...
winv:register_inventory("home", {
	...
	formspec_function = function(player)
		...
		if homesdata then
			...
			for i, hname in ipairs(otable) do
				local homelist = homesdata[hname]
				local yshift = 2 * (i - 1)
				local icon_bg = "gui_bg_curved.png"
				local icon_bg_pressed = "gui_bg_hover_curved.png"
				local colorize_arrows = ""
				...
				homes_form = homes_form..
					...
					"label[2.1,"..(1.0 + yshift)..";"..hname.."\n("..ccore.round(homelist.x, 1)..", "..ccore.round(homelist.y, 1)..", "..ccore.round(homelist.z, 1)..")]"..
					"image_button[4.575,"..(0.375 + yshift)..";0.7,0.7;gui_paintbrush.png;winv_homes_color_"..hname..";]"..
					"image_button[5.375,"..(0.375 + yshift)..";0.7,0.7;gui_location.png;winv_homes_wp_"..hname..";]"..
					waypoint_buttons..
		...
		local formspec = {
			...
			"tooltip[winv_home_add;Add a home \nYour homes limit: "..homes_limit.." homes\nPosition based on your standing location \nFill field on the left with home naming]",
	...
	on_player_receive_fields = function(player, formname, fields)
		...
		if fields.winv_home_add then
			add_home(player, fields.winv_home_name)
			winv.refresh(player)
		elseif fields.winv_home_add_wp then
			add_home(player, fields.winv_home_name, "waypoint")
